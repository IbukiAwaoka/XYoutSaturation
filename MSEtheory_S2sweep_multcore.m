...
% 並列プールの起動（既に起動していなければ）
if isempty(gcp('nocreate'))
    parpool; % 利用可能なコア数で自動起動
end

fprintf('並列計算開始（進捗表示は10個ごと）\n');

% 並列ループで計算
parfor idx = 1:num_ratios
    S2 = S2_S1_ratio(idx) * S1;
    
    % 各ワーカーで必要な計算を実行
    [MSE_final, Q_final, r_final] = computeMSEforS2(S2, S1, tEnd, rho, xi, mu, sgm_g, PRS, RNG);
    
    MSE_results(idx) = MSE_final;
    Q_results(idx) = Q_final;
    r_results(idx) = r_final;
    
    % 10個ごとに進捗表示（順序はバラバラだが参考に）
    if mod(idx, 10) == 0
        fprintf('%d/%d 完了\n', idx, num_ratios);
    end
end
...
% 結果をファイルに保存
fname = char(['multcore_MSE_S2sweep_parallel_t=', num2str(tEnd), '_mu=', num2str(mu), ...
    '_S1=', num2str(S1), '.txt']);
...
plot_fname = fullfile(outputDir, ['MSE_S2sweep_parallel_t', num2str(tEnd), '_S1', num2str(S1), '.png']);
...
%% 進捗表示用のコールバック関数
function updateProgressCallback(progress_data)
    persistent counter
    if isempty(counter)
        counter = 0;
    end
    ...
    if mod(counter, 10) == 0 || counter == progress_data.total
        fprintf('完了: %d/%d (%.2f%%)\n', counter, progress_data.total, (counter/progress_data.total)*100);
    end
end