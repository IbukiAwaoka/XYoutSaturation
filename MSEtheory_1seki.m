...
fname = char(['dyhouwa1int', ', tEnd=', num2str(tEnd), ', mu=', num2str(mu), ...
...
function dydt = odefun(t, y)
    ...
    r = max(y(2), 1e-12); % rが負値にならないように制限 (※論文のコードママですが、rは負になり得るので注意が必要かも)
...
function result = integratedXMiddle(y, x1, x2, dycov, invcov)
    % -x1 から x1 までの x に関する積分結果 (引数名は x1, x2 となっていますがコード上は対称区間として処理されている模様)
    global S2
    ...
    erf_arg1 = sqrt(a./2) .* (x1 + b.*y./a); % 上限 S1側
    erf_arg2 = sqrt(a./2) .* (-x1 + b.*y./a); % 下限 -S1側
