...
fname = char(['dyhouwa1intS1loop', ',tEnd=', num2str(tEnd), ',mu=', num2str(mu), ',S1=', num2str(S1), ',S2=', num2str(S2),',xi=', num2str(xi), '.txt']);
...
for i = 1:length(S1loop)
    ...
    % ODEソルバーで時間発展を計算
    options = odeset('RelTol', 1e-9, 'AbsTol', 1e-9);  % 精度の調整
