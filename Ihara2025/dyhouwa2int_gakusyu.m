...
fname = char(['dyhouwa tMSE', ',tEnd=', num2str(tEnd), ',mu=', num2str(mu), ',S1=', num2str(S1), ',S2=', num2str(S2), '.txt']);
Fid = fopen(fname, 'w');
Header = '#time          Q              r            MSE';
fprintf(Fid, '%s\n', Header);
...
for k = 1:sizet
    ...
    time = t(k);
      ...
      fun1=@(d,y) (-S1).*(-S2)./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun2=@(d,y) (-S1).*y./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun3=@(d,y) (-S1).*S2./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun4=@(d,y) d.*(-S2)./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun5=@(d,y) d.*y./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun6=@(d,y) d.*S2./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun7=@(d,y) S1.*(-S2)./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun8=@(d,y) S1.*y./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      fun9=@(d,y) S1.*S2./(2.*pi.*sqrt(det(dycov))).*exp(-(invcov(1,1).*d.^2+2*invcov(2,1).*d.*y+invcov(2,2).*y.^2)/2);
      
    
    fy2 = S2^2 - S2 * sqrt(2 * rho^2 * Q(k) / pi) * exp(-S2^2 / (2 * rho^2 * Q(k))) + (rho^2 * Q(k) - S2^2) * erf(S2 / sqrt(2 * rho^2 * Q(k)));
...
function dydt = odefun(t, y)
    global fd2 dfd PRS RNG mu S1 S2 xi rho sgm_g 
    dydt = zeros(2, 1);  % 2行1列ベクトルとして連立方程式を表示
    Q = y(1);
    r = y(2);

    
    fy2 = S2^2-S2*sqrt(2*rho^2*Q/pi)*exp(-S2^2/2/rho^2/Q)+(rho^2*Q-S2^2)*erf(S2/sqrt(2*rho^2*Q));
