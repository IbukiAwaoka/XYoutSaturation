...
rho = 1;
S2 = 2;
S1loop = 0:S2/100:S2*3; %初期値，刻み幅，終了値
xi = 1;
mu = 0.1;
sgm_g = sqrt(1);
...
fname = char(['dySTVMSD_S1loop', ',tEnd=', num2str(tEnd), ',mu=', num2str(mu), ',S2=', num2str(S2),',xi=', num2str(xi), '.txt']);
...
for i = 1:length(S1loop)
    S1 = S1loop(i);
    fprintf('Progress for S1 = %.2f: %.2f%%\n', S1, (i / length(S1loop)) * 100);
...
function result = calculateMSD(Q, r)
    global  sgm_g
    result = sgm_g^2-2*r+Q;
end
